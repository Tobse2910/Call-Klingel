...
//
//   phonelink-diagnose            Einmalige Diagnose, schreibt einen Bericht.
//   phonelink-diagnose --watch    Verbindet das Telefon und lauscht auf Anrufe.
//
...
head.AppendLine("PhoneLink PC - Diagnose-CLI");
...
if (args.Contains("--rfcomm"))
{
    ...
    await PhoneLinkPC.Diagnostics.Cli.RfcommProbe.RunAsync(RL, seconds);
...
if (args.Contains("--transport"))
{
    ...
    if (args.Contains("--ring"))
        await PhoneLinkPC.Diagnostics.Cli.TransportProbe.RingAsync(PL, seconds);
    else
        await PhoneLinkPC.Diagnostics.Cli.TransportProbe.RunAsync(PL);
